...
def nmf_similarity(df, num_topics):
    ...
    for topic in pbar(xrange(num_topics)):
        ...
        if len(labels[labels == topic]):
            cosine_sim = pairwise_distances(X[labels == topic], metric='cosine', n_jobs=-1)
            idx = np.tril_indices(cosine_sim.shape[0], k=-1)
            tfidf_similarity.append(cosine_sim[idx].mean())
    ...
    print '\n'
    return np.mean(tfidf_similarity), topic_similarity[idx].mean()
...
def make_similarity_plot(n_topics, tfidf_similarity, topic_similarity):
    ...
    plt.savefig('nmf_similarity.png', dpi=350)


if __name__=='__main__':
    df = pd.read_pickle('election_data.pkl')

    n_topics = range(2, 41) + range(42, 161, 2)
